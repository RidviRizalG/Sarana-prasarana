...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_GET['aksi'] ?? '') === 'tambah') {
    ...
    if ($nis === '' || $id_kategori === '' || $lokasi === '' || $ket === '') {
        header('Location: ../VIEWS/v_beranda.php?status=data_tidak_lengkap');
        exit();
    }
    ...
    if (mysqli_stmt_execute($insert)) {
        ...
        header('Location: c_beranda.php?page=histori&status=berhasil');
        exit();
    }
...
header('Location: ../VIEWS/v_beranda.php');
exit();
?>
